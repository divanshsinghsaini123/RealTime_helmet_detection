...
x = ""
# ix = 1
cap = cv2.VideoCapture("videos/helmet.mp4")  # For videos
model = YOLO("best.pt")

device = torch.device("cuda") # change to cuda for windows gpu
...
while cap.isOpened():
    ...
    results = model(new_img, stream=True, device="cuda")
    for r in results:
        boxes = r.boxes
        li = dict()
        rider_box = list()
        ...
        for i, box in enumerate(new_boxes):
            ...
            if classNames[cls] == "without helmet" and conf >= 0.5 or classNames[cls] == "rider" and conf >= 0.45 or \
                ...
                x = " This rider is without helmet"
                            ...
                            for key, value in li.items():
                                if key == f"rider{j}":
                                    if len(list(set(li[f"rider{j}"]))) == 3:
                                        x = x + " More then two riders without halmet"
                                        try:
